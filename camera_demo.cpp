...
struct Timeout {
  ...
  Timeout(std::chrono::steady_clock::duration d)
      : start(std::chrono::steady_clock::now()), duration(d) {}
  bool isTimeout() {
      ...
      case 'i':
        if (isNumber(optarg)) {
          ...
          if (options.camera_index < 0) {
            printf("[error] Invalid index of camera: %s\n", optarg);
            return false;
          }
        } else {
          printf("[error] Camera index must be a number: %s\n", optarg);
          return false;
        }
        break;
      case 'w':
        if (isNumber(optarg)) {
          ...
          printf("[error] Width must be a number: %s\n", optarg);
          return false;
        }
        break;
      case 'h':
        if (isNumber(optarg)) {
          ...
          printf("[error] Height must be a number: %s\n", optarg);
          return false;
        }
        break;
      case 'f':
...
int main(int argc, char *argv[]) {
  ...
  //    printf("[error] Open the output video file error\n");
  //    return -1;
  //  }
  ...
  while ((!timeout.isTimeout()) || (image_count != image_res_count)) {
    auto func = [&] {
      ...
      if (image_res != nullptr) {
        ...
        printf(
            "[info] image count = %d, image res count = %d, delta = %d, "
            "duration = "
            "%lldms\n",
  ...
  printf(
      "[info] Process %d frames, total time is %lldms, average frame rate is "
      "%f\n",
